...
def wait_active_element(by, element, webdriver):
    """Checks if the active element is on the screen"""
    element_taken = webdriver.find_element(by, element)
    if element_taken:
        return element_taken.is_enabled()
    return False
...
# Creating parcials
wait_button  = partial(wait_active_element, By.CSS_SELECTOR, 'button')
